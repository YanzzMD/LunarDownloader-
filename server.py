...
# Use /tmp for downloads on ephemeral filesystems (Railway, Render, etc.)
DOWNLOAD_DIR = Path(os.environ.get("DOWNLOAD_DIR", "/tmp/lunarytdl_downloads"))
DOWNLOAD_DIR.mkdir(parents=True, exist_ok=True)

# ─── Logging ──────────────────────────────────────────────────────────────────
# Log to stdout only — Railway captures stdout for log streaming
logging.basicConfig(
    level=logging.INFO,
...
def fetch_info():
    ...
    if result.returncode != 0:
        ...
        return jsonify({"error": f"Failed to fetch video info: {err_msg}"}), 400
...
def _download_worker(job_id: str, url: str, opts: dict):
    ...
    if audio_only:
        ...
        # User picked a specific video format (e.g. "137" = 1080p video-only stream).
        # YouTube separates video and audio into different streams for HD content.
        # We MUST append +bestaudio to merge audio in — otherwise the video is silent.
        # Fallback chain ensures compatibility if exact merge isn't possible.
        ...
        # quality might be a resolution-based format selector from the dropdown.
        # Same rule: always merge with best available audio.
